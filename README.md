...
- **Continue as Guest** — Skip sign-up/login entirely; the app works fully with a guest session that persists across restarts.
...
- **Fail-safe Demo Mode** — If the API key is missing or rate-limited (HTTP 429), the app seamlessly returns high-quality mock roadmaps, expansions, quizzes, and resources so the app is always demonstrable.
- **File Import** — Upload a `.txt` file **or paste raw text** into the import screen. Both paths are standardized by Gemini into a structured week/day skeleton.
...
- **Start/Pause/Resume** — New roadmaps default to `NOT_YET_STARTED`. Start to activate them. Pausing hides tasks from the dashboard; resuming shifts the calendar forward by the pause duration.
- **Auto-Activate** — Completing a task on a `NOT_YET_STARTED` roadmap automatically sets it to `ACTIVE` with today as the start date.
...
### 📅 Dashboard & Calendar
- Greets the user by name with today's date and active study streak (e.g. `Trupthi 🔥 5`).
...
- **Streak & Grace Day card**: Shows current streak, available grace days, and a "Use Grace Day" button to protect a streak on rest days.

### 📖 Roadmap Details
- Week-by-week **tab navigation** (Week 1, Week 2, ...).
- Each tab triggers a **lazy Gemini expansion** to populate full daily curriculum items on first open.
...
### ✅ Task Completion & Topic Detail
- Tap any daily task to open its **Topic Detail** screen.
- AI-generated **resource chips** (VIDEO / DOCS / PRACTICE / ARTICLE) open directly in the browser.
- Tap **Mark as Complete** to write `COMPLETED` status to Room and record a study streak log.
- Status reflects immediately on the Dashboard calendar and the weekly topic list.
- **Take Quiz** — On-demand 5-question MCQ quiz generated by Gemini, cached in Room; includes colour-coded feedback and a score screen with retake.

### 🔥 Streaks & Grace Days
- Daily study sessions are logged to `StreakLogEntity` when a topic is marked complete.
- Streak count increments if yesterday's session was logged.
- Earn **+1 Grace Day** (capped at 3) for every 7-day streak milestone.
- Tap **Use Grace Day** to insert a grace log and preserve the streak without studying.

### 🔔 Study Reminders
- `AlarmManager` exact-time notifications configured directly in Settings.
- Notification body includes a **randomized inspirational quote** + a **short summary of today's scheduled topics**.
- Reminder is automatically rescheduled to the next day after firing.
...
- Toggle **daily study reminders** on/off with an inline time picker (AM/PM format).
- Changes are saved and the reminder alarm is rescheduled immediately on Save.
...
| Screen | Description |
|---|---|
| **Auth Screen** | Tabbed Login / Sign Up + **Continue as Guest** button |
| **Dashboard** | Calendar + Today's Tasks, streak indicator, analytics cards, grace day card |
...
| **Topic Detail** | Topic view with `IN_PROGRESS` badge, AI resource chips (open in browser), Mark as Complete, Take Quiz |
| **Quiz** | 5-question MCQ per topic with colour-coded feedback, score screen, retake |
| **Notes** | Saved notes list with add/edit-dialog FAB and per-note delete |
| **Settings** | API key, study hours, daily reminder toggle + time picker |
| **AI Mentor** | Coming Soon — future multi-agent AI learning ecosystem |
...
| Bottom navigation (Dashboard, Roadmaps, AI Mentor, Settings) | ✅ Done |
...
| **Continue as Guest** (skip auth entirely) | ✅ Done |
...
| **Auto-activate roadmap** when user completes a task without manually pressing Start | ✅ Done |
...
| **Offline Demo Mode** | ✅ Done — Realistic mock data returned automatically if API key is absent or rate-limited (HTTP 429) |
...
Habit-forming retention mechanics. (Note: Spaced repetition and background WorkManager review checks were skipped to streamline core retention).

| Feature | Status |
|---|---|
| **Daily study streak** | ✅ Done — `StreakLogEntity` logged on topic completion; streak count calculated with grace day protection and milestone rewards |
| **Spaced repetition scheduling** | ⏩ Skipped |
| **Daily reminder notifications** | ✅ Done — `AlarmManager` exact-time reminders with motivational quotes and today's topic summaries in the notification body |
| **Background review alerts** | ⏩ Skipped |
